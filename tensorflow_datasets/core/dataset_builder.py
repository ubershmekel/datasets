  ...
  def _download_and_prepare(self, dl_manager, download_config=None):
    ...
    Args:
      dl_manager: (DownloadManager) `DownloadManager` used to download and cache
        data.
      download_config: `DownloadConfig`, Additionnal options.
    """
    raise NotImplementedError
...
class BeamBasedBuilder(FileAdapterBuilder):
  ...
  def _build_pcollection(self, pipeline, **kwargs):
    ...
    dataset files.
    This function is called once for each `SplitGenerator` defined in
    `_split_generators`. The examples from the PCollection will be written on
    disk.
  ...
  def _download_and_prepare(self, dl_manager, download_config):
    ...
    with beam.Pipeline(
        runner=download_config.runner,
        options=download_config.options,
    ) as pipeline:
  ...
  def _prepare_split(self, split_generator, pipeline):
    ...
    def _build_pcollection(pipeline):
      ...
      pcoll = self._build_pcollection(pipeline, **split_generator.gen_kwargs)
      pcoll |= "Encode" >> beam.Map(self.info.features.encode_example)
